...
| <a name="input_enable_public_read"></a> [enable\_public\_read](#input\_enable\_public\_read) | Set to true to grant public read access (s3:GetObject) to all objects in the bucket. Only works if enable\_policy\_creation is enabled. | `bool` | `false` | no |
