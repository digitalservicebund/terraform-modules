...
> `stackit_authorization_organization_custom_role`, `stackit_service_account_federated_identity_provider`) are part of
> the STACKIT provider's experimental `iam` feature. Enable it in the **calling** configuration's provider block, or
...
Prefer `permissions` over `roles`: the module then creates a custom role named `service-account.<name>` on the resource given
by `resource_id` (defaults to `project_id`) and assigns it to the service
account, so the pipeline only gets what it actually needs. Available permissions can be queried using stackit-cli:
